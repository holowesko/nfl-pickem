...
import { lockTimeFor, isLocked, etParts, etWallToUtc, lockWindowKey } from './time'
...
test('Thursday night football locks Thursday at 10am ET', () => {
  ...
  assert.equal(et(lockTimeFor(kickoff)), 'Thu 2026-09-10 10:00')
})
...
test('Sunday afternoon games lock Sunday at 10am ET', () => {
  ...
  assert.equal(et(lockTimeFor(kickoff)), 'Sun 2026-09-13 10:00')
})
...
test('a London game kicking at 9:30am ET locks at 7am ET that morning', () => {
  ...
  assert.equal(et(lockTimeFor(kickoff)), 'Sun 2026-10-04 07:00')
})
...
test('the early rule is driven by kickoff time, so it catches a Saturday morning game too', () => {
  ...
  assert.equal(et(lockTimeFor(kickoff)), 'Sat 2026-12-19 07:00')
})
...
test('Saturday afternoon games late in the season lock Saturday at 10am ET', () => {
  ...
  assert.equal(et(lockTimeFor(kickoff)), 'Sat 2026-12-19 10:00')
})
...
test('a Friday holiday game locks Friday morning', () => {
  ...
  assert.equal(et(lockTimeFor(kickoff)), 'Fri 2026-12-25 10:00')
})
...
test('lock times land correctly on both sides of the DST change', () => {
  ...
  assert.equal(et(lockTimeFor(beforeDst)), 'Sun 2026-10-25 10:00')
  assert.equal(et(lockTimeFor(afterDst)), 'Sun 2026-11-08 10:00')
  // And the underlying UTC instants differ by an hour, as they should.
  assert.equal(lockTimeFor(beforeDst).toISOString(), '2026-10-25T14:00:00.000Z')
  assert.equal(lockTimeFor(afterDst).toISOString(), '2026-11-08T15:00:00.000Z')
})

test('isLocked flips exactly at the deadline', () => {
  const kickoff = new Date('2026-09-13T17:00:00Z') // Sun 1pm ET
  const lock = lockTimeFor(kickoff)
  assert.equal(isLocked(kickoff, new Date(lock.getTime() - 1000)), false)
  assert.equal(isLocked(kickoff, lock), true)
})
